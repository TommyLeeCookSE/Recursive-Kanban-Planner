        ...
        for card in reg.all_cards() {
            ...
            for note in card.notes() {
                if note.title().to_lowercase().contains(&q) || note.body().to_lowercase().contains(&q) {
                    matches.push((card.id(), format!("{} (Note: {})", card.title(), note.title()), "Notes"));
                }
            }
        }
            ...
            div { class: "app-search-palette flex flex-col gap-4",
                ...
                div { class: "app-search-results flex flex-col gap-2 max-h-[60vh] overflow-y-auto",
                    if results().is_empty() && !query().is_empty() {
                        p { class: "text-center text-[var(--app-text-soft)] py-8", "No results found." }
                    }
                    ...
                    for (id, label, kind) in results() {
                        button {
                            ...
                            div { class: "flex flex-col",
                                span { class: "font-bold text-[var(--app-text-primary)] group-hover:text-[#f59e0b]", "{label}" }
                                span { class: "text-xs text-[var(--app-text-soft)] uppercase tracking-widest", "{kind}" }
                            }
