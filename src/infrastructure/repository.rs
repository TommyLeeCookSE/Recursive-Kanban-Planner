...
mod tests {
    ...
    fn test_roundtrip_integration() {
        ...
        original.create_child_card("My Task".into(), root_id, bucket_id).unwrap();

        let json = JsonRepository::serialize_registry(&original).expect("Serialization failed");
        
        let deserialized = JsonRepository::deserialize_registry(&json).expect("Deserialization failed");

        assert_eq!(original, deserialized, "Deserialized registry does not match original");
    }
}
...
impl LocalStorageRepository {
    ...
    pub fn save_to_local_storage(registry: &CardRegistry) -> Result<(), DomainError> {
        ...
        let storage = window.local_storage()
            ...
            .ok_or_else(|| DomainError::InvalidOperation("local_storage is not available".into()))?;
